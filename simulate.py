"""Simulation framework for running matchups between different strategies."""
...
class Simulation:
    """Run multiple games between strategies and track head-to-head statistics."""

    def __init__(self, debug: bool = False):
        self.results: Dict[str, Dict[str, int]] = defaultdict(
            lambda: {"wins": 0, "losses": 0, "draws": 0, "total_score": 0, "games": 0, "total_knock_score": 0, "knock_count": 0, "knock_losses": 0}
        )
        # Track head-to-head results: matchup_results[strategy1][strategy2] = {wins, losses, draws}
    ...
    def run_matchup(
        ...
        print(f"\nRunning {num_games} games: {strategy1.name} vs {strategy2.name}")
        ...
        for game_num in range(num_games):
            ...
            winner_strategies, strategy_scores, knocked_by_strat, knock_score = self._run_game(game)
    ...
    def _run_game(self, game: Game) -> Tuple[List[ComputerStrategy], Dict[ComputerStrategy, float], Optional[ComputerStrategy], float]:
        """Run a single game and return winner(s), scores, knocked_by, and knock_score."""
        game._deal_initial()
        current = 0
        knock_score = 0.0
    ...
    def _execute_strategy_turn(
        ...
        """Execute a strategy turn without any output."""
    ...
    def print_head_to_head(self, only_strategies: Optional[List[str]] = None) -> None:
        ...
        If only_strategies is provided, only print entries for those strategies.
        """
        print("\n" + "=" * 70)
        print("HEAD-TO-HEAD WIN PERCENTAGES")
        print("=" * 70)
        ...
        for strategy in all_strategies:
            ...
            overall_knock_pct = (knocks_total / games_total * 100) if games_total > 0 else 0
            ...
            for opponent in opponents:
                ...
                if total_games > 0:
                    win_pct_excl_draws = (wins / non_draw_games * 100) if non_draw_games > 0 else 0
                    draw_pct = (draws / total_games) * 100
                    knock_pct = (knocks / total_games) * 100
                    print(f"  vs {opponent:<35} Win(ex Draws) {win_pct_excl_draws:>5.1f}% | Draw {draw_pct:>5.1f}% | Knock {knock_pct:>5.1f}%")
...
def main():
    ...
    print("=" * 120)
    print(f"{'Strategy':<35} {'Win% ex Draws':>15} {'Draw %':>10} {'Avg Score':>12} {'Avg Knock':>12} {'Knock Loss %':>14}")
    print("-" * 120)
    for strategy_name in sorted(simulation.results.keys()):
        ...
        knock_loss_pct = (stats["knock_losses"] / knock_count * 100) if knock_count > 0 else 0
        print(f"{strategy_name:<35} {avg_win_pct:>15.2f} {draw_pct:>10.2f} {avg_score:>12.2f} {avg_knock:>12.2f} {knock_loss_pct:>14.2f}")
    print("=" * 120)

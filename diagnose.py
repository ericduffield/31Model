...
from computer import RandomStrategy
...
def diagnose(num_episodes: int = 10, seed: int = 42) -> None:
    ...
    for ep in range(1, num_episodes + 1):
        ...
        while not done:
            ...
            if not action_mask.any():
                print(
                    f"[Ep {ep}, Step {steps_in_episode}] ERROR: No legal actions in mask!"
                )
                no_mask_count += 1
            ...
            if was_invalid:
                invalid_count += 1
                print(
                    f"[Ep {ep}, Step {steps_in_episode}] INVALID ACTION: {action}, reward={result.reward:.3f}"
                )
        ...
        print(
            f"Episode {ep:3d} | steps={steps_in_episode:3d} | total_reward={episode_reward:.2f}"
        )
    ...
    print(
        f"Invalid actions: {invalid_count} "
        f"({100 * invalid_count / max(1, total_steps):.1f}%)"
    )
